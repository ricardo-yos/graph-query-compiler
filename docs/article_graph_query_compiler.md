...
Each regime implicitly defines which structural components are present in the query representation, such as the inclusion of filters, relational paths, or aggregation operators. In this way, regimes not only categorize query patterns but also constrain the structural composition of generated intents.
