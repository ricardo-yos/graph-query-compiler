...
The generated structured representation is the primary output of the inference stage and enables independent evaluation through schema-based metrics.
...
## 8. Schema Validation and Compilation

The structured intent generated by the language model represents an intermediate query representation between natural language and graph execution.

However, model-generated outputs cannot be directly executed. Before compilation, the generated JSON is validated against the GQC schema and semantic constraints.

This separation ensures that language model predictions remain controlled by explicit structural rules.

The validation and compilation pipeline is:
...
The validation layer verifies whether the generated intent is compatible with the predefined GQC schema.
...
- operator validity;
- filter structure;
- relationship path consistency;
- aggregation constraints;
- ranking constraints.

For example, a relational query must contain a valid graph path, while aggregation queries must define compatible aggregation operations.

Invalid representations are rejected before reaching the execution layer.

This mechanism provides an explicit control layer independent from the language model.

### 8.2 Query Compilation

After successful validation, the structured intent is transformed into an executable graph query.

The compiler performs a deterministic mapping between schema components and graph query operations:

| Structured Intent | Graph Query Component  |
| ----------------- | ---------------------- |
| target            | graph node             |
| path              | relationship traversal |
| filters           | query predicates       |
| aggregate         | aggregation clauses    |
| order_by          | sorting operations     |
| limit             | result restriction     |
| return_attributes | projection             |

Because compilation is deterministic, execution logic is separated from probabilistic language generation.

The language model is responsible only for semantic interpretation and structured intent prediction, while the compiler handles the translation into executable graph operations.

### 8.3 Architectural Role

Validation and compilation complete the GQC v1 architecture by connecting the generated schema representation to graph execution.

The main contribution of GQC v1 is the generation of a structured intermediate representation. Validation and compilation provide the mechanisms required to make this representation reliable and executable.
